...
The store holds **590** cards, of which **576** are executable and **14** are recorded but blocked. This document ranks what to build next by how much of that blocked knowledge each capability releases.

Encoding effort is measured, not guessed: across the chapters already encoded the corpus yields **0.58 cards per paragraph**, and each deferred chapter is costed from its own paragraph count at that rate. Code effort is an estimate, declared per dependency in `Rules/deferred.json` with its basis.
...
| `dep.varga-ownership` — varga ownership predicate | predicate | 1 | 1 | 1 | 1 | 21 | 0.05 |
| `dep.ashtakavarga` — ashtakavarga | calculator | 1 | 3 | 1 | 1 | 23 | 0.04 |
...
| `dep.mahadasa-ordinal` — a graha's ordinal position in the birth-fixed mahadasa sequence | predicate | 0 | 1 | 0 | 0 | 7 | 0.00 |
...
|---:|---|---|---|---:|---:|---:|
| 1 | `dep.triped-sign-class` — the 'triped' sign class | — | — | 1 | +1 | 577 |
| 2 | `dep.transit` — transit (gochara) calculation | — | — | 6 | +2 | 579 |
| 3 | `dep.second-nativity` — a second nativity | — | — | 5 | +1 | 580 |
| 4 | `dep.prashna` — prashna (horary) branch | — | — | 6 | +1 | 581 |
| 5 | `dep.adjudication` — Stage 7 adjudication | — | — | 8 | +1 | 582 |
| 6 | `dep.strength-ranking` — ordinal comparison of graha strength | `dep.shadbala-arithmetic` | — | 26 | +2 | 584 |
| 7 | `dep.universal-quantification` — universal quantification over a variable-membership set | — | — | 15 | +1 | 585 |
| 8 | `dep.manual-verification` — human verification of the encoding | — | — | 20 | +1 | 586 |
| 9 | `dep.body-part-significator` — house-to-body-part correspondence | — | — | 2 | +1 | 587 |
| 10 | `dep.varga-ownership` — varga ownership predicate | — | ch. 21 | 21 | +1 | 588 |
| 11 | `dep.ashtakavarga` — ashtakavarga | — | ch. 23 | 23 | +1 | 589 |

Completing this sequence costs **133 units** and moves executable cards from **576** to **589** — releasing **13 of the 14** currently blocked cards.
...
Across the 18 unencoded chapters that is roughly **765 cards**, of which about **0 (0%) would be inert on arrival** if they were encoded before the missing capabilities exist.
