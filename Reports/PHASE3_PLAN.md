...
The store holds **537** cards, of which **520** are executable and **17** are recorded but blocked. This document ranks what to build next by how much of that blocked knowledge each capability releases.

Encoding effort is measured, not guessed: across the chapters already encoded the corpus yields **0.69 cards per paragraph**, and each deferred chapter is costed from its own paragraph count at that rate. Code effort is an estimate, declared per dependency in `Rules/deferred.json` with its basis.
...
| `dep.ashtakavarga` — ashtakavarga | calculator | 1 | 3 | 0 | 0 | 26 | 0.00 |
...
|---:|---|---|---|---:|---:|---:|
| 1 | `dep.triped-sign-class` — the 'triped' sign class | — | — | 1 | +1 | 521 |
| 2 | `dep.vargottama` — vargottama extractor | `dep.varga` | ch. 03 | 14 | +3 | 524 |
| 3 | `dep.second-nativity` — a second nativity | — | — | 5 | +1 | 525 |
| 4 | `dep.transit` — transit (gochara) calculation | — | — | 6 | +1 | 526 |
| 5 | `dep.prashna` — prashna (horary) branch | — | — | 6 | +1 | 527 |
| 6 | `dep.dasa` — vimshottari dasa engine | — | ch. 19 | 16 | +2 | 529 |
| 7 | `dep.adjudication` — Stage 7 adjudication | — | — | 8 | +1 | 530 |
| 8 | `dep.strength-ranking` — ordinal comparison of graha strength | `dep.shadbala-arithmetic` | — | 26 | +2 | 532 |
| 9 | `dep.universal-quantification` — universal quantification over a variable-membership set | — | — | 15 | +1 | 533 |
| 10 | `dep.manual-verification` — human verification of the encoding | — | — | 20 | +1 | 534 |
| 11 | `dep.body-part-significator` — house-to-body-part correspondence | — | — | 2 | +1 | 535 |
| 12 | `dep.ashtakavarga` — ashtakavarga | — | ch. 23 | 26 | +1 | 536 |

Completing this sequence costs **145 units** and moves executable cards from **520** to **536** — releasing **16 of the 17** currently blocked cards.
...
Across the 21 unencoded chapters that is roughly **1079 cards**, of which about **348 (32%) would be inert on arrival** if they were encoded before the missing capabilities exist.
