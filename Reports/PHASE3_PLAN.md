...
The store holds **393** cards, of which **373** are executable and **20** are recorded but blocked. This document ranks what to build next by how much of that blocked knowledge each capability releases.

Encoding effort is measured, not guessed: across the chapters already encoded the corpus yields **0.63 cards per paragraph**, and each deferred chapter is costed from its own paragraph count at that rate. Code effort is an estimate, declared per dependency in `Rules/deferred.json` with its basis.
...
|---:|---|---|---|---:|---:|---:|
| 1 | `dep.vargottama` — vargottama extractor | `dep.varga` | ch. 03 | 14 | +3 | 376 |
| 2 | `dep.strength` — planetary and house strength (Stage 4) | — | ch. 04 | 34 | +7 | 383 |
| 3 | `dep.second-nativity` — a second nativity | — | — | 5 | +1 | 384 |
| 4 | `dep.transit` — transit (gochara) calculation | — | — | 6 | +1 | 385 |
| 5 | `dep.dasa` — vimshottari dasa engine | — | ch. 19 | 15 | +3 | 388 |
| 6 | `dep.prashna` — prashna (horary) branch | — | — | 6 | +1 | 389 |
| 7 | `dep.adjudication` — Stage 7 adjudication | — | — | 8 | +1 | 390 |
| 8 | `dep.universal-quantification` — universal quantification over a variable-membership set | — | — | 15 | +1 | 391 |
| 9 | `dep.manual-verification` — human verification of the encoding | — | — | 20 | +1 | 392 |
| 10 | `dep.ashtakavarga` — ashtakavarga | — | ch. 23 | 24 | +1 | 393 |

Completing this sequence costs **147 units** and moves executable cards from **373** to **393** — releasing **20 of the 20** currently blocked cards.
...
Across the 22 unencoded chapters that is roughly **1100 cards**, of which about **415 (38%) would be inert on arrival** if they were encoded before the missing capabilities exist.
