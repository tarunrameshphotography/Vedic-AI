...
The store holds **529** cards, of which **512** are executable and **17** are recorded but blocked. This document ranks what to build next by how much of that blocked knowledge each capability releases.

Encoding effort is measured, not guessed: across the chapters already encoded the corpus yields **0.68 cards per paragraph**, and each deferred chapter is costed from its own paragraph count at that rate. Code effort is an estimate, declared per dependency in `Rules/deferred.json` with its basis.
...
|---:|---|---|---|---:|---:|---:|
| 1 | `dep.triped-sign-class` — the 'triped' sign class | — | — | 1 | +1 | 513 |
| 2 | `dep.vargottama` — vargottama extractor | `dep.varga` | ch. 03 | 14 | +3 | 516 |
| 3 | `dep.second-nativity` — a second nativity | — | — | 5 | +1 | 517 |
| 4 | `dep.transit` — transit (gochara) calculation | — | — | 6 | +1 | 518 |
| 5 | `dep.prashna` — prashna (horary) branch | — | — | 6 | +1 | 519 |
| 6 | `dep.dasa` — vimshottari dasa engine | — | ch. 19 | 16 | +2 | 521 |
| 7 | `dep.adjudication` — Stage 7 adjudication | — | — | 8 | +1 | 522 |
| 8 | `dep.strength-ranking` — ordinal comparison of graha strength | `dep.shadbala-arithmetic` | — | 26 | +2 | 524 |
| 9 | `dep.universal-quantification` — universal quantification over a variable-membership set | — | — | 15 | +1 | 525 |
| 10 | `dep.manual-verification` — human verification of the encoding | — | — | 20 | +1 | 526 |
| 11 | `dep.body-part-significator` — house-to-body-part correspondence | — | — | 2 | +1 | 527 |
| 12 | `dep.ashtakavarga` — ashtakavarga | — | ch. 23 | 25 | +1 | 528 |

Completing this sequence costs **144 units** and moves executable cards from **512** to **528** — releasing **16 of the 17** currently blocked cards.
...
Across the 21 unencoded chapters that is roughly **1065 cards**, of which about **344 (32%) would be inert on arrival** if they were encoded before the missing capabilities exist.
