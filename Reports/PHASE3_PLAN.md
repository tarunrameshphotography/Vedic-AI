...
The store holds **554** cards, of which **540** are executable and **14** are recorded but blocked. This document ranks what to build next by how much of that blocked knowledge each capability releases.

Encoding effort is measured, not guessed: across the chapters already encoded the corpus yields **0.61 cards per paragraph**, and each deferred chapter is costed from its own paragraph count at that rate. Code effort is an estimate, declared per dependency in `Rules/deferred.json` with its basis.
...
| `dep.transit` — transit (gochara) calculation | calculator | 3 | 4 | 2 | 2 | 6 | 0.33 |
| `dep.second-nativity` — a second nativity | schema | 1 | 2 | 1 | 1 | 5 | 0.20 |
| `dep.prashna` — prashna (horary) branch | engine | 1 | 1 | 1 | 1 | 6 | 0.17 |
| `dep.adjudication` — Stage 7 adjudication | engine | 1 | 14 | 1 | 1 | 8 | 0.12 |
...
| `dep.varga-ownership` — varga ownership predicate | predicate | 1 | 1 | 1 | 1 | 22 | 0.05 |
...
|---:|---|---|---|---:|---:|---:|
| 1 | `dep.triped-sign-class` — the 'triped' sign class | — | — | 1 | +1 | 541 |
| 2 | `dep.transit` — transit (gochara) calculation | — | — | 6 | +2 | 543 |
| 3 | `dep.second-nativity` — a second nativity | — | — | 5 | +1 | 544 |
| 4 | `dep.prashna` — prashna (horary) branch | — | — | 6 | +1 | 545 |
| 5 | `dep.adjudication` — Stage 7 adjudication | — | — | 8 | +1 | 546 |
| 6 | `dep.strength-ranking` — ordinal comparison of graha strength | `dep.shadbala-arithmetic` | — | 26 | +2 | 548 |
| 7 | `dep.universal-quantification` — universal quantification over a variable-membership set | — | — | 15 | +1 | 549 |
| 8 | `dep.manual-verification` — human verification of the encoding | — | — | 20 | +1 | 550 |
| 9 | `dep.body-part-significator` — house-to-body-part correspondence | — | — | 2 | +1 | 551 |
| 10 | `dep.varga-ownership` — varga ownership predicate | — | ch. 21 | 22 | +1 | 552 |
| 11 | `dep.ashtakavarga` — ashtakavarga | — | ch. 23 | 24 | +1 | 553 |

Completing this sequence costs **135 units** and moves executable cards from **540** to **553** — releasing **13 of the 14** currently blocked cards.
...
Across the 19 unencoded chapters that is roughly **880 cards**, of which about **0 (0%) would be inert on arrival** if they were encoded before the missing capabilities exist.
