...
The store holds **407** cards, of which **387** are executable and **20** are recorded but blocked. This document ranks what to build next by how much of that blocked knowledge each capability releases.

Encoding effort is measured, not guessed: across the chapters already encoded the corpus yields **0.66 cards per paragraph**, and each deferred chapter is costed from its own paragraph count at that rate. Code effort is an estimate, declared per dependency in `Rules/deferred.json` with its basis.
...
| `dep.strength` — planetary and house strength (Stage 4) | calculator | 8 | 12 | 7 | 7 | 41 | 0.17 |
...
| `dep.adjudication` — Stage 7 adjudication | engine | 1 | 8 | 1 | 1 | 8 | 0.12 |
...
|---:|---|---|---|---:|---:|---:|
| 1 | `dep.vargottama` — vargottama extractor | `dep.varga` | ch. 03 | 14 | +3 | 390 |
| 2 | `dep.strength` — planetary and house strength (Stage 4) | — | ch. 04 | 35 | +7 | 397 |
| 3 | `dep.second-nativity` — a second nativity | — | — | 5 | +1 | 398 |
| 4 | `dep.transit` — transit (gochara) calculation | — | — | 6 | +1 | 399 |
| 5 | `dep.dasa` — vimshottari dasa engine | — | ch. 19 | 16 | +3 | 402 |
| 6 | `dep.prashna` — prashna (horary) branch | — | — | 6 | +1 | 403 |
| 7 | `dep.adjudication` — Stage 7 adjudication | — | — | 8 | +1 | 404 |
| 8 | `dep.universal-quantification` — universal quantification over a variable-membership set | — | — | 15 | +1 | 405 |
| 9 | `dep.manual-verification` — human verification of the encoding | — | — | 20 | +1 | 406 |
| 10 | `dep.ashtakavarga` — ashtakavarga | — | ch. 23 | 25 | +1 | 407 |

Completing this sequence costs **150 units** and moves executable cards from **387** to **407** — releasing **20 of the 20** currently blocked cards.
...
Across the 22 unencoded chapters that is roughly **1140 cards**, of which about **429 (38%) would be inert on arrival** if they were encoded before the missing capabilities exist.
