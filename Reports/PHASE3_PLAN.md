...
The store holds **624** cards, of which **609** are executable and **15** are recorded but blocked. This document ranks what to build next by how much of that blocked knowledge each capability releases.

Encoding effort is measured, not guessed: across the chapters already encoded the corpus yields **0.55 cards per paragraph**, and each deferred chapter is costed from its own paragraph count at that rate. Code effort is an estimate, declared per dependency in `Rules/deferred.json` with its basis.
...
| `dep.middle-of-sign-scope` — what "middle of a sign" means for a non-hermaphrodite graha | concept | 1 | 2 | 1 | 1 | 4 | 0.25 |
...
| `dep.distinct-graha-quantifier` — a condition-language test that two bound variables resolve to different grahas | combinator | 0 | 1 | 0 | 0 | 4 | 0.00 |
...
| `dep.lagna-strength` — strength verdict for the Lagna/Ascendant | predicate | 0 | 3 | 0 | 0 | 5 | 0.00 |
| `dep.own-or-benefic-dignity-in-varga` — own-sign / benefic dignity read against a divisional placement | predicate | 0 | 1 | 0 | 0 | 8 | 0.00 |
...
| `dep.sign-to-country` — which country/place a sign or divisional sign indicates | reference | 0 | 1 | 0 | 0 | 2 | 0.00 |
...
| `dep.weakest-of-comparator` — rank/compare N named candidates and identify the weakest | combinator | 0 | 2 | 0 | 0 | 5 | 0.00 |
...
graph LR
  ...
  middle_of_sign_scope --> degree_range
  own_or_benefic_dignity_in_varga --> dignity
  own_or_benefic_dignity_in_varga --> varga
...
| 4 | `dep.middle-of-sign-scope` — what "middle of a sign" means for a non-hermaphrodite graha | `dep.degree-range` | — | 4 | +1 | 614 |
| 5 | `dep.second-nativity` — a second nativity | — | — | 5 | +1 | 615 |
| 6 | `dep.prashna` — prashna (horary) branch | — | — | 6 | +1 | 616 |
| 7 | `dep.adjudication` — Stage 7 adjudication | — | — | 8 | +1 | 617 |
| 8 | `dep.strength-ranking` — ordinal comparison of graha strength | `dep.shadbala-arithmetic` | — | 26 | +2 | 619 |
| 9 | `dep.universal-quantification` — universal quantification over a variable-membership set | — | — | 15 | +1 | 620 |
| 10 | `dep.varga-ownership` — varga ownership predicate | — | ch. 21 | 20 | +1 | 621 |
| 11 | `dep.manual-verification` — human verification of the encoding | — | — | 20 | +1 | 622 |
| 12 | `dep.body-part-significator` — house-to-body-part correspondence | — | — | 2 | +1 | 623 |
| 13 | `dep.ashtakavarga` — ashtakavarga | — | ch. 23 | 22 | +1 | 624 |

Completing this sequence costs **136 units** and moves executable cards from **609** to **624** — releasing **15 of the 15** currently blocked cards.
...
Across the 16 unencoded chapters that is roughly **673 cards**, of which about **0 (0%) would be inert on arrival** if they were encoded before the missing capabilities exist.

## Cards that no sequence here releases

7 card(s) remain blocked after every step above, because they depend on capabilities with no path that pays for itself yet:
