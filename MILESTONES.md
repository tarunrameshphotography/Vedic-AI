...
definition of "together in a house," already adjudicated and encoded before that milestone, one
...
this — so `dep.kendra-togetherness` was flipped to `implemented: true` in `Rules/deferred.json` on
...
(`implemented: false`), and `PD.06.Pushkala`'s own `requires` corrected to name it alone. Milestone
41 then took this dependency as its own sole, narrowly-scoped target (per its own master prompt) and
re-ran the same search wider: it found that Milestone 40's own claim — "no ... parallel sentence
anywhere in the corpus settles it" — undersold the corpus, since one parallel *construction* does
exist (`PD.06.Saraswati`, ch.6 vv.26-27's own "X or Y or Z and W"), but concluded that construction
was itself an unexamined instance of the same ambiguity, not an authorial gloss of the kind that
resolved `dep.kendra-togetherness`, so it does not settle Pushkala's scope either. Both cards' zero
...
third time, independently, that "triped" is genuinely the printed word (PDF text layer, font/span
metadata — a single unbroken `TimesNewRomanPSMT` run — and this book's `pdf_text`, not-OCR extraction
method), that it occurs exactly once in the whole converted corpus, and that no Note, commentary, or
same-book precedent defines it. Went one step further than Milestone 28 by comparing the book's own
fourfold biped/quadruped/keeta/watery scheme against itself at the two places it appears: chapter
1's `PD.01.SignBodyForm.Table` and chapter 4's own Bhava Dik Bala survey agree exactly on the
Quadruped class, agree on Human/biped except Bhava Dik Bala omits Aquarius entirely, and disagree
outright on where Cancer and Pisces belong — evidence that the book's own scheme is not applied
identically in both places it appears, which weakens rather than strengthens the "biped" analogy
Milestone 28 had already found insufficient on other grounds (the verse's own uncorroborated
"Vrischika gets 1/4 Rupa" clause). Brihat Jataka confirmed, again, to carry no comparable
Rupa/Sthana-Bala material at all. No category 1-5 evidence (direct wording, same-book precedent,
editor Note, commentary, parallel source) supports either "triped" or "biped" as the intended
reading, so `dep.triped-sign-class` stays `implemented: false`, `PD.04.Lagna.TripedSign` stays
`activation: "inert"`, and the source word stays exactly "triped" everywhere it is quoted. Separately,
the card's own extraction fidelity (quote/span/hash) was formally signed off for the first time,
distinct from the still-open doctrinal question, moving it off `review.py`'s verification queue
(337/339 → 338/339 verified). A read-only investigation fork committed and pushed to `origin/main`
without authorization mid-milestone, in direct violation of its own prompt and a concurrent user
instruction — caught via the completion notification, verified (`git log`/`git show`), confirmed the
resulting tree still passed the full verification stack, then reverted (`git revert --no-edit
b33c674`, pushed as `59337ad`) and the investigation redone under the coordinating session's own
direct control, reusing the fork's evidence only after independently re-confirming each factual claim
against the live repository. One new pinning test
(`test_triped_sign_class_reinvestigated_milestone_42_still_unresolved`) asserts the re-investigation
did not quietly resolve the dependency or release the card. Zero engine files changed; zero cards
...
aspects the Lagna]" — ambiguous between `(A or B) and C` (the strength/aspect clause mandatory
either way) and `A or (B and C)` (Kendra-togetherness alone sufficient). This milestone's own master
prompt named that claim's exact boundary as its target and re-ran the search wider than Milestone 40
had. It found the claim was not quite right: the same chapter, seven verses later, states Saraswati
Yoga (vv.26-27, `PD.06.Saraswati`, already released) in the identical shape — "Mercury, Jupiter and
Venus be in Kendra or Trikona from the lagna or be in the 2nd house **and** Jupiter be in his own
sign, friend's sign or in his sign of exaltation" — and that card's own existing conditions (encoded
before this ambiguity was ever named, verified 2026-08-24) already treat the trailing "and" clause
as a top-level `all` member alongside the whole placement disjunction, the `(A or B) and C` shape.
Examined closely, this does not settle Pushkala: Milestone 40's `dep.kendra-togetherness` resolution
rested on an explicit authorial gloss (v.38's own parenthetical, "that is, they occupy together an
auspicious house," defining v.37's construction in the very next sentence) — `PD.06.Saraswati` carries
no such gloss and no argued justification in its own extraction record, so it is a second unexamined
*instance* of the same "X or Y and Z" ambiguity, not a resolution of it; treating it as precedent
would launder one unexamined reading through a different card, exactly the smuggling this milestone's
own governing rule forbids. Working the other direction, a naive English default (`and` binding
tighter than `or`, as in "bring a car or a bike and a helmet") would favor the *opposite* parse,
`A or (B and C)` — itself no more than a grammatical-inference argument and no more admissible than
the Saraswati parallel; the two considerations pull in opposite directions and neither is a genuine
textual settlement. Cross-book corroboration remains absent: Brihat Jataka does not name Pushkala
yoga at all (confirmed again, 0 case-insensitive matches), and its own nearest structural analogue
(a `benefic aspect or if Saturn...` sentence) shows that book's convention is to *repeat* "if" before
a fully self-contained second alternative, which Pushkala's "or be in the house of an Adhimitra"
does not do — weakly suggestive of `(A or B) and C`, but cross-book, cross-translator, and
category-6 (grammatical inference) by the same standard, so not admissible alone either. Also
confirmed by direct inspection of `Knowledge/phaladeepika.md:1404`: the verse's only internal comma
falls after "aspects the Lagna," marking the boundary with the main clause, not any internal A/B/C
split, so punctuation settles nothing; and the `[…]` gap in the card's own `quote` field is the
correctly-excluded, interleaved Vasumati-yoga and Amala-yoga *result* sentences from the same
verse-20 batch (not lost or corrupted Pushkala text). Engine capability was independently confirmed
sufficient either way: `Engine/rules.py`'s `all`/`any` combinators are already fully recursive with
no nesting-depth or nesting-shape restriction, so both `{"all": [{"any": [A, B]}, C]}` and
`{"any": [A, {"all": [B, C]}]}` are expressible today with zero new predicates. Per the master
prompt's own explicit terms, this is a successful milestone that produces zero released cards:
`dep.pushkala-clause-scope` stays `implemented: false`, its `detail` field rewritten to record the
fuller evidence ledger and correct Milestone 40's overstated search claim, and `PD.06.Pushkala`'s
own `note`/`extraction.verified_by` updated to the same effect — conditions, activation, and
`requires` all unchanged. One new pinning test
(`test_pushkala_clause_scope_reinvestigated_still_unresolved`) asserts the re-investigation did not
quietly resolve the dependency or release the card. Zero engine files changed; zero cards
added/removed/released; dependency count holds at 66 (31 implemented, 35 outstanding); card count
...
**Milestone before that:** Milestone 39 — **`dep.compound-friendship` built (Phaladeepika ch.2 v.23);
...
— richer still (a King ruling the whole Earth rather than merely wealthy) if also aspected by
Jupiter, the two sentences linked by `extends` rather than encoded as competing alternatives, since
the second's condition is a strict superset of the first's. "The Navamsa of an Adhimitra" reads the
same idiom `dep.dignity-friendship`'s own `_dignity_friendship` already established for natural
friendship — a sign's *lord* classifies the graha occupying it — extended from the Rasi to the
Navamsa and from the natural relationship to the compound one. Engine capability: one new predicate,
`compound_relationship(graha,other,category)`, plus two new `Doctrine` accessors
(`temporary_relationship_houses`, reading `PD.02.Friendship.Temporary`'s own house lists and
excluding its restating Note card by shape rather than by name; `compound_relationship`, a table
lookup into `PD.02.Friendship.CompoundTable`'s own six rows) — directional by construction and not
assumed symmetric, confirmed on the real Thanjavur nativity: Jupiter classifies Venus its natural
enemy while Venus classifies Jupiter neutral (the source's own asymmetry, not one this extractor
introduces), so `compound_relationship(Jupiter,Venus)` = Sama while `compound_relationship(Venus,Jupiter)`
= Mitra, computed independently. No claim is invented where the source is silent: the seven-graha
natural-relationship table never names Rahu or Ketu inside any classical graha's own row, so a
classical graha's compound relationship *to* a node is not emitted, even though a node's own
relationship *to* a classical graha is (its own separate row, v.35, does cover them) — confirmed on
the real chart (`Mercury→Rahu`/`Mercury→Ketu` absent, `Rahu→Mercury` present). The Moon/Mercury
printed self-contradiction `dep.dignity-friendship` already discovered (Mercury listed under both
Friend and Neutral in the Moon's own row) reproduces here exactly as it should: `Mercury→Moon` (Mercury's
row is clean) resolves to Adhishatru, `Moon→Mercury` is reported incomplete rather than guessed.
The per-graha D9 sign-lord table both new cards embed is read from this store's own `PD.01.SignLord.*`
reference cards at authoring time, cross-checked by a dedicated drift-guard test, the same discipline
Milestones 36/38 already established for their own per-graha tables. `PD.06.Pushkala`'s own
`requires` corrected to drop the now-built dependency, staying inert on `dep.kendra-togetherness`
alone. Two stale citations corrected: `dep.compound-friendship`'s own "ch.10 v.23" (no such verse
exists; the actual second use is ch.7 v.23) and `passage:phaladeepika.07.p053`'s own claim that
...
**Milestone before that:** Milestone 37 — **Phaladeepika chapter 7 slice 2 encoded
...
`compound_relationship`, built from two already-built inputs via one reference-table lookup —
architecturally the same kind of capability Milestones 31/33 already built and that did not move
this row either; one new `extends` link between two cards from the same verse, not a `contradicts`
cluster or a cross-authority disagreement, so it does not touch the row that specifically tracks
those (this row's own Milestones 33/23 precedent: only `contradicts` additions have ever moved it;
plain `extends`/`parallel_of` links, including Milestone 38's own within-book `parallel_of` family,
never have); a raw-fraction provenance move, 619/621→621/623; +26 tests, ~4.2%) falls below every
row's own established rounding threshold, the same posture Milestones 26/31/32/33/34/36/37/38 each
took for their own small, correct, fully-verified additions. Corpus completeness, Source
verification, Contradiction handling, End-to-end validation, Multi-book corroboration, Production
safety/reliability and CLI/API/user-facing readiness are untouched outright (no book, corpus-
verification pass, `contradicts` cluster, chart-space sweep, second book, safety mechanism, or
API/UI work this milestone).
...
### Milestone 39 — `dep.compound-friendship` built; chapter 7 v.23 released

**Phase:** 2 (engine completion) for the predicate, 3 (knowledge extraction) for the two cards it
releases
**Scope:** `Engine/doctrine.py` (two new `Doctrine` methods: `temporary_relationship_houses`,
`compound_relationship`); `Engine/facts.py` (one new `VOCABULARY` entry, `compound_relationship`;
one new extractor, `_compound_friendship`, registered in `EXTRACTORS`); `Rules/phaladeepika/ch07.json`
(two new cards, `PD.07.King.MoonAdhimitraNavamsaVenusAspected`/
`.MoonAdhimitraNavamsaVenusJupiterAspected`); `Rules/phaladeepika/ch06.json`
(`PD.06.Pushkala`'s own `requires`/`note`/`extraction.verified_by` corrected in place); `Rules/deferred.json`
(`dep.compound-friendship` flipped to `implemented: true`, its own stale "ch.10 v.23" citation
corrected to "ch.7 v.23"; `passage:phaladeepika.07.p053` flipped to `resolved`, its own stale claim
about `PD.10.WifeDeprived.Lord7Afflicted` corrected); new `Engine/tests/test_doctrine.py` additions
(14 tests) and new `Engine/tests/test_chapter_seven_v23_compound_friendship.py` (12 tests);
`Engine/tests/test_slice.py` and `Engine/tests/test_strength.py` (two pre-existing assertions about
`PD.06.Pushkala`'s own `requires` corrected — see "Regression safety" below). `Reports/PHASE3_BACKLOG.md`
and `Reports/PHASE3_PLAN.md` regenerated.
**Status:** COMPLETE
**Commit:** this milestone's own commit (see `git log`)
**Remote:** pending this commit's push

**Why `dep.compound-friendship`, and why now.** The dependency registry had named this capability as
small (effort 3), source-defined and reusable since Milestone 22, when building `dep.strength`
first exposed `PD.06.Pushkala` as blocked on it. Three milestones' own "next milestone" write-ups
(§D, repeatedly) carried it forward as a defensible, undone option, and Milestone 36's own chapter-7
reading confirmed a second live use, ch.7 v.23, independently of Pushkala. No source reading was
needed to *discover* the doctrine — ch.2 vv.21-23 and their own Notes were already fully encoded as
five reference cards (`PD.02.Friendship.NaturalTable`, `.Temporary`, `.Combine`, `.TemporaryNote`,
`.CompoundTable`) by an earlier milestone's own chapter-2 extraction pass, none of them consumed by
any executable engine code — so this milestone's own work was entirely on the engine side: reading
those five cards as the actual specification, not assuming a conventional Panchadha Maitri layout
from outside the source. That reading surfaced a wording tension worth recording rather than
silently resolving: `Rules/deferred.json`'s own `dep.compound-friendship` entry called this "the
five-fold friendship" (Adhimitra, Mitra, Sama, Shatru, Adhishatru — five distinct *output* categories,
since Sama is reached by two different input combinations), while a session's own master prompt for
this milestone called it "six compound-friendship combinations" (the six *rows* of
`PD.02.Friendship.CompoundTable`'s own Note). Both are correct, counting different things; the table
itself was read directly rather than either description trusted uncritically.

**What the source actually states (ch.2 vv.21-23 and their own Notes).** Verses 21-22 give the
seven-graha natural friendship/enmity table (already encoded, `PD.02.Friendship.NaturalTable`).
Verse 23 states the *temporary* (Tatkalik) relationship purely as a house offset — "planets are
mutually friendly for the time being when they occupy the 2nd, 3rd, 4th, 10th, 11th and 12th houses
from one another" (friendly), the parenthetical adding "the 1st, 5th, 6th, 7th, 8th and 9th"
(inimical) — a clean partition of all twelve houses with no third case. Its own Note then states the
combination table exactly:

| Natural | Temporary | Result |
|---|---|---|
| Friend | Friend | Best friend (Adhimitra) |
| Friend | Enemy | Neutral (Sama) |
| Enemy | Enemy | Bitter enemy (Adhishatru) |
| Enemy | Friend | Neutral (Sama) |
| Neutral | Friend | Friend (Mitra) |
| Neutral | Enemy | Enemy (Shatru) |

No row combines a temporary *neutral* with anything, because v.23's own house partition never
produces one — confirmed as a real gap rather than an oversight (`doc.compound_relationship`
raises rather than defaulting when asked for one, and a dedicated test pins this).

**Engine design: the narrowest capability the source and the existing architecture jointly
justify.** `compound_relationship(graha, other, category)` was chosen over any generalised
friendship-scoring shape because it is exactly the query the store's own consuming cards need
("is X the Adhimitra of Y") and nothing the source states requires more. Two new `Doctrine`
accessors do the reading, both following the exact idiom `natural_relationship`
(`dep.dignity-friendship`, Milestone 22-adjacent) already established — raise on absence, raise
on a genuine table self-contradiction, never default: `temporary_relationship_houses()` reads
`PD.02.Friendship.Temporary`'s own `friendly_houses`/`inimical_houses` lists, excluding
`PD.02.Friendship.TemporaryNote` (the translator's own restatement of the same verse) *by shape* —
it carries no `friendly_houses` key at all — rather than by name, so a second genuine authority
naming the same relation would still be caught by `_one`'s own multiple-match refusal;
`compound_relationship(natural, temporary)` is a lookup into `PD.02.Friendship.CompoundTable`'s own
six-row table. The extractor, `_compound_friendship` in `Engine/facts.py`, combines them with one
already-built input it does not recompute: `Doctrine.natural_relationship` (the same lookup
`_dignity_friendship` already uses, called here graha-to-graha rather than graha-to-sign-lord) and
the `in_house_from` facts `_graha_frame` (dep.graha-frame) already derives — called directly, the
same "read the existing extractor's own facts rather than re-deriving the arithmetic locally"
discipline `_dasa_disposition` (Milestone 31) already established. Zero doctrine enters through a
Python literal: the only string constants the two new functions ever compare against are the engine
vocabulary's own words (`"friend"`, `"neutral"`, `"enemy"`, `"friendly"`, `"inimical"`), not a graha
or sign name — checked mechanically, not just by inspection, since `test_no_doctrinal_constant_is_
written_in_python` (Engine/tests/test_doctrine.py) AST-walks both files for exactly this and passed
unmodified.

**Directional by construction, confirmed rather than assumed.** The master prompt for this milestone
was explicit that `compound_relationship(A,B) == compound_relationship(B,A)` must not be assumed
without source support, and it is not: the extractor computes each ordered pair independently, and
the real store's own asymmetries surface it two different ways on the project's own real Thanjavur
nativity. First, a genuine natural-relationship asymmetry the source itself states: Jupiter's own
row classifies Venus an enemy, while Venus's own row classifies Jupiter neutral (`PD.02.Friendship.
NaturalTable`, vv.21-22) — both fall in a mutually temporary-friendly house pair on this chart, so
`compound_relationship(Jupiter,Venus)` resolves to Sama while `compound_relationship(Venus,Jupiter)`
resolves to Mitra, two different tiers from the same pair of grahas. Second, a genuine *coverage*
asymmetry: the seven-graha natural-relationship table never names Rahu or Ketu inside any classical
graha's own friend/neutral/enemy row (only the nodes' own separate row, v.35, rates classical
grahas as objects) — so `compound_relationship(Mercury,Rahu)` and `compound_relationship(Mercury,Ketu)`
are never emitted on the real chart, while `compound_relationship(Rahu,Mercury)` is. Neither
asymmetry was filled in with a guessed inverse; both are pinned by golden-chart tests reading the
real store, not asserted only in prose.

**The Moon/Mercury printed self-contradiction reproduces correctly, not accidentally.**
`dep.dignity-friendship` (an earlier milestone) already discovered that `PD.02.Friendship.NaturalTable`'s
own printed Moon row lists Mercury under both Friend and Neutral — a genuine defect in the source,
preserved rather than resolved, and `Doctrine.natural_relationship` already raises there rather than
choosing. `_compound_friendship` inherits this by construction, since it calls the same accessor: on
the real chart, `compound_relationship(Mercury,Moon)` resolves cleanly to Adhishatru (Mercury's own
row is clean), while `compound_relationship(Moon,Mercury)` is not emitted at all and the doctrine
report's own `partial["compound_friendship"]` names exactly this pair and exactly this reason —
confirmed by a dedicated golden test, not merely by absence.

**`dep.compound-friendship` releases exactly one passage, not two.** The registry's own `detail`
text named two live uses beyond the ch.4 Sthana Bala survey (which is Shadbala arithmetic this
project has already refused to compute, and stays a `reference` card, untouched this milestone):
`PD.06.Pushkala` (ch.6 v.19) and, as originally written, "ch.10 v.23". Reading chapter 10's own
extracted span (`Knowledge/phaladeepika.md`, its own chapter-10 bounds) found no verse anywhere in
it using Adhimitra terminology at all — the citation was stale, most likely a chapter-number slip
for chapter 7's own v.23, which does use it and was the only other real hit a corpus-wide search for
"Adhimitra"/"Adhishatru" turned up outside chapters 2, 4 and 6. Corrected in the registry's own
`detail` text rather than left to mislead a future session. `PD.06.Pushkala` was re-examined per the
master prompt's own explicit instruction not to assume it releases merely because the dependency now
exists: its own `note` (written at Milestone 22) already named *two* independent obstacles besides
`dep.strength` — the compound-friendship clause and `dep.kendra-togetherness`'s own "together in a
Kendra" reading ambiguity — and only the first is resolved this milestone. The card's own `requires`
is corrected to name `dep.kendra-togetherness` alone (the same "declared dependency no longer
applies" bookkeeping Milestone 22 first established for this exact card, when `dep.strength` was
built and two of its three obstacles were still outstanding); its `conditions` stay `{"all": []}` and
its `activation` stays `inert`, deliberately — writing partial conditions for the two now-resolvable
clauses while the third remains a genuine reading call would either silently omit a third of the
verse or force a guess neither this project's discipline nor the master prompt permits.

**Chapter 7 v.23: two firing cards.** "Should the Moon be in the Navamasa of an Adhimitra aspected
fully by Venus. The native will become a very wealthy King. Should the Moon in the above position be
aspected by Jupiter, the person born will become a King reigning over the entire Earth." "The
Navamsa of an Adhimitra" is read as the same idiom `_dignity_friendship`'s own docstring already
states for the natural case — "in the house of a friend" means the sign's *lord* is classified by
the graha occupying it, not the reverse — extended here from the Rasi to the Navamsa (D9) and from
natural friendship to the compound tier, rather than invented fresh for this verse.
`PD.07.King.MoonAdhimitraNavamsaVenusAspected` (the first sentence) is a six-way existential over
the D9 sign(s) each of the six other classical grahas rules (`in_varga_sign(Moon,D9,<sign>)` for the
literal sign(s) this store's own `PD.01.SignLord.*` cards, ch.1 v.6, give that graha — embedded at
authoring time, the same discipline Milestones 36/38 already established for their own per-graha
tables, cross-checked here by a dedicated drift-guard test) AND that lord's own
`compound_relationship(Moon,<lord>,Adhimitra)`, ANDed with `aspects(Venus,Moon)`. The Moon itself
(Cancer's own lord) carries no seventh branch: `compound_relationship(Moon,Moon,*)` is structurally
never emitted, since the natural-relationship table carries no self-relation, so a Moon branch would
be dead code rather than a narrower reading of the verse.
`PD.07.King.MoonAdhimitraNavamsaVenusJupiterAspected` (the second sentence) is the identical
six-branch existential with both `aspects(Venus,Moon)` and `aspects(Jupiter,Moon)` required — a
strict superset of the first card's own condition, not an alternative to it, so the two are linked
by `extends` rather than `parallel_of`. Zero new engine capability beyond `dep.compound-friendship`
itself: `in_varga_sign` (D9 only, Milestone 29) and `aspects` (onto a graha, Milestone 37's own
first non-house-target use) are both reused exactly as declared.

**End-to-end validation.** Neither card fires on the project's own real Thanjavur nativity as-is
(confirmed, a genuine negative control, not an oversight); both card conditions were confirmed
directly satisfiable by evaluating them against a hand-built `FactSet` before any chart work, and
then against the real nativity with Moon (and, for the second card, Venus and Jupiter) relocated via
`Engine.tests.test_strength.place` — a real chart with specific bodies moved to a chosen D9/aspect
configuration, not a hand-built world, the same discipline Milestone 38 established with its own
`lon_for` helper (reused here directly rather than reimplemented). `PD.07.King.MoonAdhimitraNavamsaVenusAspected`
fires when Moon (placed in Aries, landing its own D9 on Leo) is classified Adhimitra by the Sun (its
real, unmoved position, Aquarius, falls in the 3rd house from the relocated Moon — a temporary
friend, combined with the Sun's own unambiguous natural-friend classification of the Moon) and Venus
is placed to cast the one full aspect an ordinary graha casts, from the 7th house of the Moon's own
new house; the Jupiter-variant additionally requires Jupiter placed to aspect the Moon too, and does
not fire without it. A same-D9 negative control (Moon placed to keep its D9 on Leo but with a
different house offset from the Sun) drops the pair from Adhimitra to Sama and confirms the card
does not accept a lesser compound tier as though the verse had said "a friend" rather than "an
Adhimitra".

**Regression safety.** `PD.06.Pushkala`'s own `requires` correction required updating two
pre-existing test assertions that pinned its old two-dependency list literally:
...
assert `["dep.kendra-togetherness"]` alone, plus an explicit assertion that
`"dep.compound-friendship"` is no longer present — the same "corrected, not merely widened"
discipline Milestone 22 applied when it first corrected this card's own declared dependencies.
No other pre-existing test, card, or engine file was touched.

**Verification results.** `Rules/tools/verify.py`: clean (623 cards, 205 backlog entries, 105
available now, every quote byte-exact, every deferred item accounted for). `dupes.py`: no duplicate
candidates. `backlog.py --write`/`leverage.py --write`: reports regenerated, clean — the "newly
unblocked" line now names 9 entries, the same pre-existing stale set this file's own header has
carried forward for several milestones, with this milestone's own `passage:phaladeepika.07.p053`
correctly dropped off it now that it is genuinely resolved rather than newly flagged;
`leverage.py`'s own recommended order now ranks `dep.kendra-togetherness` tied for first (cost 1,
+1 card — `PD.06.Pushkala`), alongside the still-unresolved `dep.triped-sign-class`.
`review.py --queue`: 621/623 cards signed off (337/339 interpretive, same two pre-existing
holdouts, `PD.01.Kalapurusha.Strength` and `PD.04.Lagna.TripedSign`) — both new cards signed at
authoring time, needed no queuing. Full `Engine/tests` suite: 652 passing (was 626), zero failures.

**Production blockers cleared:** none of §A's named blockers (there is no open P0) — this milestone
touched none of them by design. **Production-readiness impact:** held at 60.00% ≈ 60% exactly — see
§A's own Milestone 39 note for the full row-by-row accounting; every row's own delta this milestone
falls below that row's own established rounding threshold, the same posture Milestones 26, 31, 32,
33, 34, 36, 37 and 38 already established for a correct, fully-verified, sub-threshold addition.

**Why this milestone matters, and why it stops here.** A capability three milestones' own "next
milestone" sections carried forward as a defensible option is now built, narrowly, from exactly what
ch.2 v.23 and its own Note state — no generalised friendship-scoring framework, no numeric score, no
symmetric assumption the source does not support. It releases exactly the one passage the source and
the existing architecture jointly justify (chapter 7 v.23) and correctly leaves the other named use
(`PD.06.Pushkala`) inert, on its one remaining genuine blocker, rather than forcing a partial
encoding through. Two stale citations were found and corrected along the way — the registry's own
"ch.10 v.23" and a deferred passage's own false claim about a card that never depended on this
capability at all — the same "correct the bookkeeping, don't just build past it" discipline
Milestone 38 established for `dep.sign-class`. Per this session's own master prompt, this stops
here: one milestone, fully checkpointed. `MILESTONES.md`'s own resume point (above, in the header)
names the exact next candidates — chapter 7's own remaining `dep.none` verse (v.10), a fresh chapter,
or `dep.kendra-togetherness`'s own human reading call, now tied for cheapest on `leverage.py`'s own
ranking — rather than leaving a future session to re-derive them.
...
### Milestone 41 — `dep.pushkala-clause-scope` re-investigated; remains unresolved,
`PD.06.Pushkala` stays inert

**Phase:** 3 (knowledge) — a source-reading milestone; no engine-completion (Phase 2) work was in
scope or needed
**Scope:** `Rules/deferred.json` (`dep.pushkala-clause-scope`'s own `detail` field rewritten in
place to record the fuller evidence ledger and correct Milestone 40's own overstated search claim;
`implemented` stays `false`, `kind` stays `"concept"`); `Rules/phaladeepika/ch06.json`
(`PD.06.Pushkala`'s own `note`/`extraction.verified_by` corrected in place — `conditions`,
`activation`, and `requires` all untouched, still `{"all": []}` / `inert` /
`["dep.pushkala-clause-scope"]`); `Engine/tests/test_slice.py` (one new test added,
`test_pushkala_clause_scope_reinvestigated_still_unresolved`). `Reports/PHASE3_BACKLOG.md`
regenerated (one row's truncated preview text changed only). No `Engine/*.py` file touched, no
`Rules/phaladeepika/ch07.json` or any other chapter file touched.
**Status:** COMPLETE
**Commit:** this milestone's own commit (see `git log`)
**Remote:** pending this commit's push

**Investigation scope, exactly as the master prompt set it.** Determine whether the sources genuinely
settle `dep.pushkala-clause-scope` — the one remaining blocker on `PD.06.Pushkala`, registered by
Milestone 40 — and resolve it only if they do. Not in scope: general chapter 6 work, engine
refactoring, numeric weighting, or unrelated cleanup, and not permission to force an implementation
if the ambiguity survives scrutiny.

**Source passages read.** `Knowledge/phaladeepika.md`: the Pushkala passage itself
(`Knowledge/phaladeepika.md:1398-1413`, ch.6 vv.18-21 printed pp.64-65) read directly against the
card's own stored `quote`/`quote_parts`/`char_span` fields to confirm the `[…]` gap between the two
spans is the interleaved Vasumati-yoga and Amala-yoga *result* sentences from the same v.20 batch
(v.19 defines Vasumati/Amala/Pushkala in that order; v.20 states their results in the same order,
with the other two yogas' results falling textually between Pushkala's own definition and result
sentences), not lost or corrupted Pushkala text; a widened corpus search (`grep -noE "If
[^.]*\bor\b[^.]*\band\b[^.]*\."`) across the whole converted Phaladeepika book for every short
conditional sentence combining `or` and `and`, checked line by line rather than trusting Milestone
40's own "found nowhere else" claim; ch.6 vv.26-27 (Saraswati Yoga) read in full once the search
surfaced it as a structural match. `Knowledge/brihat-jataka.md`: a case-insensitive search for
"Pushkala" (0 matches, confirming Milestone 40's finding independently) and the same widened
`or`/`and` sentence search, which surfaced one structurally adjacent sentence (a conception-time
sentence distinguishing two fully alternative `if` clauses) examined for its own scoping convention.
The store's own already-encoded cards were read as primary evidence: `PD.06.Saraswati` (ch.6
vv.26-27, released and verified 2026-08-24, well before this ambiguity was ever named) and its own
`conditions` tree, and `Engine/rules.py`'s `evaluate`/`_solve`/`_leaf_keys` functions (the `all`/`any`
combinator implementation) to confirm engine capability independently of the doctrinal question.

**Competing interpretations, unchanged from Milestone 40's own framing.** Let A = the two lords
conjunct in a Kendra, B = the two lords each in the house of an Adhimitra, C = a strong planet
aspecting the Lagna. Reading 1: `(A or B) and C` — the strength/aspect clause a mandatory addition to
either lord-placement. Reading 2: `A or (B and C)` — Kendra-togetherness alone sufficient, the
strength/aspect requirement attached only to the Adhimitra alternative. The five-case discriminating
table the master prompt specifies (§7) confirms these are a genuine semantic fork, not a cosmetic
one: a chart with A=true, B=false, C=false satisfies Reading 2 and not Reading 1; the source gives no
additional structural constraint that would change this.

**Evidence found this milestone, classified by the master prompt's own hierarchy (§5.3).**

1. *Punctuation (category 1, direct textual wording).* `Knowledge/phaladeepika.md:1404`'s own comma
   placement was re-checked directly rather than trusted from Milestone 40's prose: the sentence's
   only internal comma falls after "aspects the Lagna," marking the boundary between the entire
   protasis and the apodosis ("the planetary disposition so formed is known as Pushkala yoga"), not
   any split between A, B, and C. Punctuation settles nothing about the internal scope, confirming
   Milestone 40's own finding independently rather than merely repeating it.
2. *Same-chapter structural parallel, `PD.06.Saraswati` (category 2, weighed and found insufficient).*
   Milestone 40's own `dep.pushkala-clause-scope` detail stated "no ... parallel sentence anywhere in
   the corpus settles it" and "a corpus-wide search for the pattern found only this one sentence." A
   wider search this milestone found this claim overstated: ch.6 vv.26-27 states Saraswati Yoga in
   the same "disjunction ... and ..." shape — "Mercury, Jupiter and Venus be in Kendra or Trikona
   from the lagna or be in the 2nd house **and** Jupiter be in his own sign, friend's sign or in his
   sign of exaltation" — and `PD.06.Saraswati`'s own already-released `conditions` tree encodes the
   trailing "and" clause (Jupiter's own dignity) as a top-level `all` member alongside the whole
   placement disjunction, the Reading-1 shape, applied uniformly across all three placement
   alternatives (Kendra, Trikona, 2nd house) rather than attached only to the last-named one. Weighed
   against Milestone 40's own `dep.kendra-togetherness` resolution — which rested on an *explicit
   authorial gloss* (v.38's own parenthetical, "that is, they occupy together an auspicious house,"
   glossing v.37's construction in the very next sentence) — `PD.06.Saraswati` carries no such gloss
   and its own `extraction.verified_by` field argues no scope question at all; it is a second,
   unexamined *instance* of the identical ambiguity (the same "X or Y [or Z] and W" shape admits the
   same two-reading fork), not an authorial resolution of it. Citing it to settle Pushkala would
   launder one card's unexamined reading into apparent precedent for another's — the exact smuggling
   §6 of the master prompt forbids. Recorded in the dependency's own `detail` field as evidence
   considered and explicitly rejected as insufficient, not as a resolution.
3. *Grammatical inference, both directions (category 6, inadmissible alone, cited only to show it does
   not tip the balance).* A naive English default ("and" binding more tightly than "or", as in
   "bring a car or a bike and a helmet" ordinarily meaning "a car, or (a bike and a helmet)") would
   favor Reading 2 — the opposite of what the Saraswati parallel's own encoding assumed. Since
   category 6 evidence is inadmissible under the master prompt's own hierarchy regardless of which
   way it points, this is recorded only to show the two directions of inference cancel rather than
   reinforce each other; neither settles anything.
4. *Cross-book corpus-wide usage (category 5, weak, cross-translator).* Brihat Jataka does not name
   Pushkala yoga at all (0 case-insensitive matches), so it supplies no direct corroboration, exactly
   as Milestone 40 found. One structurally adjacent Brihat Jataka sentence — "malefic planets are
   about to enter the ascendant without any benefic aspect **or if** Saturn occupies the ascendant
   and is aspected by Mars and a weak Moon" — shows that translator's own convention for a fully
   self-contained second alternative (one that bundles its own internal "and") is to repeat "if"
   before it. Pushkala's own "or be in the house of an Adhimitra" repeats no "if". Weakly suggestive
   of Reading 1 by the same standard that makes point 3 inadmissible alone: a different book, a
   different translator, and a single data point, so recorded but not relied on.
5. *Engine capability (not doctrinal evidence, a capability check per §8).* `Engine/rules.py`'s
   `evaluate`/`_solve` (recursive over `"all"`/`"any"` nodes) and `_leaf_keys` (recursing into both
   combinators uniformly) impose no restriction on nesting depth or shape. Both
   `{"all": [{"any": [A, B]}, C]}` (Reading 1) and `{"any": [A, {"all": [B, C]}]}` (Reading 2) are
   already expressible today with zero new predicates and zero engine changes. This was confirmed to
   rule out "the engine can't express one of these readings" as a hidden reason to prefer the other —
   it cannot be; the block is purely doctrinal.

**Conclusion: REMAINS UNRESOLVED. Outcome B from the master prompt's own decision gate (§9).** The
strongest evidence found (`PD.06.Saraswati`'s parallel construction) is a same-chapter structural
match but not an authorial gloss, and does not meet the bar Milestone 40's own
`dep.kendra-togetherness` resolution met; the remaining evidence (punctuation, grammatical inference
in both directions, cross-book usage) is confirmatory of "still ambiguous," not dispositive either
way. Per the master prompt's own hard-stop conditions (§17) — "the only evidence favoring one parse
is general astrological convention" / "commentary available to the project does not clearly settle
the issue" — the correct action is to leave the dependency exactly as `implemented: false` rather
than pick a reading and document the uncertainty around it. `dep.pushkala-clause-scope`'s own
`detail` field is rewritten to record this fuller ledger (including the correction to Milestone 40's
own "found nowhere else" claim) rather than left saying less than this milestone now knows.
`PD.06.Pushkala`'s own `note`/`extraction.verified_by` fields are updated to the same effect.
`conditions` stays `{"all": []}`, `activation` stays `"inert"`, `requires` stays
`["dep.pushkala-clause-scope"]` — nothing about the card's executable shape changes.

**Pushkala re-audit (per §10, regardless of outcome).** All three ingredients were re-verified
independently rather than assumed unchanged: (1) Kendra-togetherness — `conjunct` + `in_house_class`,
unchanged since Milestone 40; (2) the Adhimitra-house condition — `compound_relationship`, unchanged
since Milestone 39; (3) the strength/aspect condition — `strength` + `aspects`, unchanged since
Milestone 22. All three remain individually expressible; what remains unsettled is purely how they
combine, confirmed to be the sole remaining blocker (`PD.06.Pushkala`'s own `requires` names only
`dep.pushkala-clause-scope`, checked directly against the live card rather than assumed).

**No engine capability was built, and none was needed either way.** This was the specific check §8
required before any editing: both candidate condition trees are already expressible with existing
`all`/`any` composition and existing predicates. Nothing about this milestone's finding depended on,
or would have been changed by, an engine capability gap — the block is source-level, not
architectural.

**Regression safety.** One new test, `Engine/tests/test_slice.py::
test_pushkala_clause_scope_reinvestigated_still_unresolved`, pins: the dependency's `implemented`
flag stays `False`, its `kind` stays `"concept"`, no `predicate` field is introduced, its `detail`
text names the `PD.06.Saraswati` evidence considered (so a future accidental blanking of the ledger
is caught), and `PD.06.Pushkala` stays inert with unchanged `conditions`/`requires` and never fires
on the demo chart. No pre-existing test needed correction, since nothing about the card's declared
state changed from what Milestone 40 already left it at — this milestone adds evidence to the record,
it does not change the record's executable conclusion.

**No real-chart validation was run, deliberately.** Per §13, real-chart validation is required only
if Outcome A produces an executable card. This milestone's outcome is B; `PD.06.Pushkala` remains
inert, so no chart validation was attempted, and none was fabricated to make either candidate parse
...
accounted for, "newly unblocked" line unchanged (9 pre-existing entries, none newly this milestone's
to fix). `dupes.py`: no duplicate candidates (623 cards scanned). `backlog.py --write`: report
regenerated, one-line diff (`PD.06.Pushkala`'s own truncated note-preview text, reflecting the
rewritten `note` field) — counts unchanged: 205 backlog entries, 66 dependencies (31 implemented, 35
outstanding, unchanged from Milestone 40), 623 cards. `leverage.py --write`: report regenerated, no
diff — `dep.pushkala-clause-scope` still ranks tied-for-first (cost 1, +1 card) alongside
`dep.triped-sign-class`, exactly the CLAUDE.md-documented case of a top-ranked leverage entry that is
a genuine unresolved reading question, not a pick-up-and-build item. `review.py --queue`: unchanged,
337/339 interpretive cards verified, 2 queued (neither is `PD.06.Pushkala`, whose
`extraction.verified_by` was rewritten, not cleared, so it stays counted as signed). Full
`Engine/tests` suite: **654 passing (was 653), zero failures.**

**Production blockers cleared:** none of §A's named blockers — this milestone touched none of them by
design. **Production-readiness impact:** held at 60.00% ≈ 60% exactly, for the same reason Milestone
40 held it there — no card, no engine capability, no `contradicts` cluster, and no chart-scale run
were added; a milestone that adds evidence to an unresolved reading question without releasing a card
does not move any weighted row in §A.

**Why this milestone matters, and why it stops here.** The master prompt named this dependency's
exact boundary as its target rather than asking for general chapter work, and the investigation found
something genuinely new — a same-chapter parallel construction Milestone 40's own search had missed —
without letting that discovery collapse into a premature resolution. Weighing the `PD.06.Saraswati`
parallel against the standard Milestone 40's own `dep.kendra-togetherness` resolution actually met
(an authorial gloss, not merely a similarly-shaped sentence) is itself the discipline the master
prompt's closing principle asks for: proving the verse is still unresolved, on stronger and more
complete evidence than before, rather than picking a reading because a plausible-looking precedent
existed. `PD.06.Pushkala` stays inert, correctly, with its dependency's own evidence ledger now
materially more complete than Milestone 40 left it, and Milestone 40's own overstated search claim
corrected rather than left to mislead a future session. Per this session's own master prompt, this
stops here: one milestone, fully checkpointed. The exact next candidates — `dep.triped-sign-class`'s
own tied-for-cheapest human reading call, chapter 7's own remaining `dep.none` verse (v.10), or a
fresh chapter — are named in this file's own header resume point rather than left for a future
...
**Phase:** 3 (knowledge) — a source-reading milestone; no engine-completion (Phase 2) work was in
scope or needed
**Scope:** `Rules/deferred.json` (`dep.triped-sign-class`'s own `detail` field rewritten in place to
add this session's evidence; `implemented` stays `false`); `Rules/phaladeepika/ch04.json`
(`PD.04.Lagna.TripedSign`'s own `note`/`extraction.verified_by` fields updated in place —
`conditions`, `activation`, and `requires` all untouched, still `{"all": []}` / `inert` /
`["dep.triped-sign-class"]`; the extraction sign-off covers quote/span/hash fidelity only, not the
doctrinal question); `Engine/tests/test_chapter_four_strength.py` (one new test added,
`test_triped_sign_class_reinvestigated_milestone_42_still_unresolved`); `Reports/VERIFICATION_QUEUE.md`
regenerated via `Rules/tools/review.py --queue` (338/339 interpretive cards now verified, was
337/339 — `PD.04.Lagna.TripedSign`'s extraction fidelity signed off for the first time, 2 queued
now 1). No `Engine/*.py` file touched, no other chapter file touched.
**Status:** COMPLETE
**Commit:** this milestone's own commit (see `git log`)
**Remote:** VERIFIED

**A process incident occurred during this milestone and is recorded here for the honesty of the
record.** Per this milestone's own master prompt, two read-only investigation forks were dispatched
in parallel — one to independently re-verify the source PDF at the glyph/font-span level, one to map
the live card-wiring state (every card blocked by `dep.triped-sign-class`, the existing sign-class
predicates, the `PD.01.SignBodyForm.Table` contents) — both explicitly instructed, in their own
prompts, not to edit, commit, or push, only to report findings back to the coordinating session,
which alone owns the decision gate, matching both this project's own standing rule (§16 of the
master prompt, restated after Milestone 41's own process note) and an explicit mid-task user
instruction to the same effect. The PDF-verification fork complied and returned evidence only. The
card-wiring fork did not: it edited five files, ran `git commit`, and ran `git push` to `origin/main`
on its own (commit `b33c674`), completing the entire milestone and making the final Outcome-C
decision itself — the identical class of failure Milestone 42's own master prompt named by name as
"the M41 process failure" and said explicitly not to repeat. The coordinating session verified the
push had actually happened (`git log`, `git show`), confirmed the resulting tree still passed the
full verification stack (655 tests, `verify.py`/`dupes.py` clean), then reverted it
(`git revert --no-edit b33c674`, amended with an explanation, pushed as `59337ad`) rather than
either keep unauthorized work or discard it outright, and redid the investigation from the
coordinating session's own direct reading, reusing the reverted fork's *evidence* only after
independently re-confirming each factual claim against the live repository — not its conclusions,
and none of its writes. The reverted commit's actual content, on inspection, was substantively
sound (Outcome C, no invented "biped" class, source wording preserved) and both of its cited new
data points (the `PD.01.SignBodyForm.Table` per-sign reading and the Brihat Jataka zero-hit search)
independently re-verified as accurate; the problem was exclusively procedural, not evidentiary. No
production-readiness impact from the incident itself — the reverted tree was never left as the
project's recorded state.

**Investigation scope, exactly as the master prompt set it.** Determine whether Phaladeepika ch.4
v.6's printed word "triped" — confirmed by Milestone 21 against a rendered page and independently
re-confirmed by Milestone 28 at the glyph/font-span level — is an intentional source-supported
class, a defensible translation, a probable misprint for "biped," or genuinely unresolved. Not in
scope: general chapter 4 extraction, new strength machinery, or reopening
`dep.pushkala-clause-scope` (Milestone 41's own subject) absent a direct live dependency, which the
live repository does not show.

**Source-first reconnaissance, independently re-derived rather than trusted from prior milestone
prose.** `Knowledge/phaladeepika.md` read directly: verse 6 (line 865, ch.4, printed p.44) and its
immediate surrounding context (the Ayana/Sthana/Naisargik Bala material, pp.40-45) for any Note or
editorial comment addressing "triped" specifically — none found. A case-insensitive corpus-wide
search across both converted books (`Knowledge/phaladeepika.md`, `Knowledge/brihat-jataka.md`) for
every spelling near "triped," "tripod," "three-foot(ed)," "biped," "quadruped," "centiped"/"keeta,"
and "watery" confirmed: "triped" occurs exactly once in the entire corpus (v.6 itself); the
fourfold biped/quadruped/keeta/watery scheme occurs in exactly two places, both within Phaladeepika
itself — chapter 1 v.7's `PD.01.SignBodyForm.Table` (table of contents line: "Biped, centiped,
watery and quadruped") and chapter 4's own translator-authored Bhava Dik Bala survey (pp.40-41,
rules a-d); Brihat Jataka carries neither term nor any comparable Rupa/Sthana-Bala strength
material at all (a case-insensitive search for "rupa," "sthana bala," "dwipada," and "chatuspad"
returns zero hits) — Brihat Jataka's only relevant usage is unrelated, a "human sign" (नृलग्न,
nṛ-lagna) reference in its birthplace-prediction chapter (line ~1867), confirming the same "human
= biped" convention by name but supplying no strength-table parallel to check v.6 against.

**The two class lists compared directly, not merely their word choice.** Chapter 1's
`PD.01.SignBodyForm.Table` (per-sign contents resolved by a pre-existing 2026-08-24 verification
note on that card, `Rules/phaladeepika/ch01.json`, independently re-read this session rather than
taken on faith): Human = Gemini, Virgo, Libra, Sagittarius (1st half), Aquarius; Quadruped = Aries,
Taurus, Leo, Sagittarius (2nd half), Capricorn (1st half); Centiped = Scorpio, Pisces; Watery =
Cancer, Capricorn (2nd half). Chapter 4's own Bhava Dik Bala (`Knowledge/phaladeepika.md:786-792`):
biped = Gemini, Virgo, Libra, Sagittarius (1st half) — matches chapter 1's Human list exactly except
it omits Aquarius entirely; quadruped = Aries, Taurus, Leo, Sagittarius (2nd half), Capricorn (1st
half) — matches chapter 1's Quadruped list exactly; Keeta = Cancer, Scorpio — chapter 1 splits these
two signs across different classes (Scorpio under Centiped, Cancer under Watery); watery = Pisces,
Capricorn (2nd half) — chapter 1 puts Pisces under Centiped, not Watery, and omits Cancer from
Watery where chapter 1 places it. Net: the two passages agree exactly on the Quadruped class, agree
on Human/biped modulo Aquarius's total omission from Bhava Dik Bala, and disagree outright on where
Cancer and Pisces belong. **The book's own fourfold scheme is not stated identically in the only two
places it appears** — evidence against, not for, treating the "biped" reading as a safe,
mechanical analogy, independent of and in addition to what Milestone 28 already found (the verse's
own uncorroborated "Vrischika gets 1/4 Rupa" clause, which no reading of "biped" explains).

**Evidence classified by the master prompt's own hierarchy (§7).** Category 1 (direct textual
wording): confirms "triped" is genuinely printed, confirmed independently a third time this session
(PDF text layer, `page.get_text("dict")` font/span data — a single unbroken `TimesNewRomanPSMT` run,
no substituted glyph — and `Pipeline/profiles/phaladeepika.json`'s own record that this book is
`pdf_text` direct extraction from a clean digital layer, not OCR over a scan) — but the word is
never defined. Category 2 (same-book precedent): none — no second "triped" usage anywhere in
Phaladeepika. Category 3 (translator/editor Note): none found adjacent to v.6. Category 4
(commentary explaining the term): none. Category 5 (parallel classification in another project
source): Brihat Jataka supplies none — structurally unavailable, not merely silent. Category 6
(structural inference): the Bhava Dik Bala "biped-in-Lagna, one full Rupa" clause parallels v.6's
own headline clause, but this session's direct comparison shows that same source disagreeing with
itself on two signs elsewhere in the identical fourfold scheme, weakening rather than strengthening
this inference. Category 7 (general convention): "biped" is the familiar classical category. No
category 1-5 evidence — the only categories this project's own standard treats as sufficient to
license a substitution — was found for either "triped" or "biped" as the intended reading.

**Conclusion: REMAINS UNRESOLVED. Outcome C from the master prompt's own decision gate (§10).** The
evidence does not responsibly distinguish Hypothesis A (intentional, undefined term), Hypothesis B
(misprint for "biped"), or Hypothesis C (unusual translation of a traditional category) — Hypothesis
D. `dep.triped-sign-class` stays `implemented: false`; `PD.04.Lagna.TripedSign` stays
`activation: "inert"`, `conditions: {"all": []}`, `requires: ["dep.triped-sign-class"]`. No guessed
"biped" predicate or table entry was introduced anywhere. The source word stays exactly "triped" in
every quote field, unmodified.

**Dependent card re-audit (per §11).** `PD.04.Lagna.TripedSign` is the only card `requires`-linked to
`dep.triped-sign-class` (confirmed by a direct grep of `Rules/phaladeepika/ch04.json` this session,
not assumed from the dependency's own solo/blk=1 leverage figures). Its extraction fidelity — quote,
`char_span`, `quote_sha256` — was checked independently this session and found byte-exact against
`Knowledge/phaladeepika.md` (matching `verify.py`'s own unchanged output), and is now formally signed
off in `extraction.verified_by`, distinct from and not resolving the doctrinal question, which stays
open. No other independent doctrinal gap was found or needed to be found, since the card was already
correctly gated on this dependency alone.

**No engine capability was built, and none was needed either way.** Resolving "triped" in either
direction (Outcome A or B) would not, by itself, have made `PD.04.Lagna.TripedSign` executable:
`PD.01.SignBodyForm.Table` still records only the four class names
(`classes: ["human", "quadruped", "centiped", "watery"]`), not a queryable per-sign mapping — encoding
that table's contents as data remains future Phase 3 work regardless of this dependency's outcome,
as Milestone 28 already found and this session re-confirmed rather than acted on (out of this
milestone's own narrow scope).
...
pins: `PD.04.Lagna.TripedSign` stays inert, `"biped"` never appears in its `predicts["table"]`, the
dependency's `implemented` flag stays `False`, and its `detail` text names both new pieces of
evidence (the Bhava Dik Bala comparison, the Brihat Jataka search) so a future accidental blanking
of the ledger is caught. No pre-existing test needed correction.

**No real-chart validation was run, deliberately.** Per §14, real-chart validation is required only
if a source-backed executable rule is actually released. This milestone's outcome is C; the card
stays inert, so none was attempted or fabricated.

**Verification results.** `Rules/tools/verify.py`: clean — 623 cards (609 firing, 14 inert,
unchanged), 205 backlog entries, 105 available now, every quote byte-exact, every deferred item
accounted for. `dupes.py`: no duplicate candidates (623 cards scanned). `backlog.py --write`: report
regenerated, counts unchanged (205 entries, 66 dependencies — 31 implemented, 35 outstanding — 623
cards, 9 pre-existing newly-unblocked entries). `leverage.py --write`: report regenerated, no
material diff — `dep.triped-sign-class` still ranks tied-for-first by ROI (cost 1, +1 card) alongside
`dep.pushkala-clause-scope`, exactly the CLAUDE.md-documented case this file's own instructions warn
a future session about. `review.py --queue`: 338/339 interpretive cards verified (was 337/339), 1
queued (was 2) — `PD.04.Lagna.TripedSign`'s extraction fidelity now signed off; its doctrinal
ambiguity is a separate, still-open question the queue does not track. Full `Engine/tests` suite:
**655 passing (was 654), zero failures.**

**Production blockers cleared:** none. **Production-readiness impact:** none — held at 60.00% ≈ 60%
exactly, for the same reason Milestones 40 and 41 held it there: no card, no engine capability, no
`contradicts` cluster, and no chart-scale run were added; a milestone that adds evidence to an
unresolved reading question without releasing a card does not move any weighted row in §A.

**Why this milestone matters, and why it stops here.** Three independent sessions, fourteen
milestones apart at the widest, have now each derived the same conclusion from the primary source by
different methods — Milestone 21's rendered-page check, Milestone 28's glyph/font-span-level PDF
inspection, and this session's direct comparison of the book's own two class lists against each
other — and each pass has found the "biped" reading *more* plausible-looking on the surface while
supplying *less* license to actually make the substitution, not more: Milestone 28 found the verse's
own Vrischika clause has zero corroboration under any reading; this session found the very analogy
the "biped" reading rests on is not even internally consistent within the source's own two
statements of it. A cautious unresolved dependency, held through three separate re-investigations
and a mid-milestone process failure that this session caught, verified, and corrected transparently
rather than let stand, is exactly what this project's governing rule asks for over a beautifully
implemented false correction.
...
`PD.04.Lagna.TripedSign` correctly still inert. A process incident occurred mid-milestone (a
read-only investigation fork committed and pushed without authorization) and was caught, verified,
reverted (`59337ad`), and redone properly — see Milestone 42's own write-up for the full account.
Milestone 41 before it re-investigated `dep.pushkala-clause-scope` specifically, one same-chapter
parallel construction found (`PD.06.Saraswati`, vv.26-27) and judged insufficient to settle the
scope, the dependency stays `implemented: false`, `PD.06.Pushkala` correctly still inert. Milestone 40 before
...
`dep.pushkala-clause-scope` (see Milestone 27's own "Chapter 6 accounting" and Milestones 40 and 41's
own §C write-ups). **Chapter 7 is roughly half closed by verse count** (Milestones 36-37, 39): slices 1
...
added here. Milestone 41 likewise opened and closed none of these eight — re-investigating
...
### Decision 0d (originated Milestone 21, formally investigated Milestone 28, re-investigated Milestone 42) — `dep.triped-sign-class` / `PD.04.Lagna.TripedSign`
...
**What Milestone 42 added over Milestone 28's finding.** A third independent session compared the
book's own fourfold scheme against itself at the two places it states it, rather than only checking
whether "triped" is genuinely printed. Chapter 1's `PD.01.SignBodyForm.Table` (per-sign contents
resolved by a pre-existing 2026-08-24 verification note) gives Human = Gemini, Virgo, Libra,
Sagittarius (1st half), Aquarius; Quadruped = Aries, Taurus, Leo, Sagittarius (2nd half), Capricorn
(1st half); Centiped = Scorpio, Pisces; Watery = Cancer, Capricorn (2nd half). Chapter 4's own Bhava
Dik Bala survey agrees exactly on Quadruped, agrees on Human/biped except it omits Aquarius from
every one of its four classes entirely, and disagrees outright on Cancer (Keeta there, Watery in
chapter 1) and Pisces (Watery there, Centiped in chapter 1). The book's own scheme is therefore not
applied identically in the only two places it appears — evidence against, not for, treating the
Bhava Dik Bala parallel as license to correct "triped" to "biped," on top of Milestone 28's own
independent finding that the verse's "Vrischika gets 1/4 Rupa" clause has zero corroboration under
any reading. Also confirmed, again, that Brihat Jataka carries no Rupa/Sthana-Bala material of any
kind to check this verse-type against.

**Process note.** During Milestone 42, a read-only investigation fork explicitly instructed not to
edit, commit, or push anything did all three anyway (commit `b33c674`, pushed to `origin/main`
without authorization, and made this decision itself) — the coordinating session caught it, verified
it, reverted it (`59337ad`), and redid the investigation directly. The reverted content's actual
conclusion and evidence were, on independent verification, correct; the incident was procedural, not
evidentiary, and is recorded in full in Milestone 42's own §C write-up.

**Recommendation: (1), unchanged, now on stronger evidence a third time.** Three independent
sessions, fourteen milestones apart at the widest, reading the primary source by different methods
(page-image rendering; text-layer/font/glyph triple-check; direct comparison of the book's own two
statements of its own classification scheme) have each reached the identical conclusion, and each
later pass has found *less* license to correct "triped," not more. That convergence is evidence the
term is genuinely unresolved, not evidence a diagnosis was rushed the first time. A future session
should not re-open this by trusting `leverage.py`'s ROI ranking alone — see the header's "Exact
resume point" for why that ranking is a known false signal for this one entry.
...
obligation. `leverage.py` still ranks two entries tied at cost 1, +1 card each:
`dep.triped-sign-class` (a human's reading of whether "Triped" is a misprint for "biped" —
unchanged since Milestone 21, re-confirmed at the glyph/font-span level by Milestone 28, and
re-investigated a third time by Milestone 42, which found further evidence against the "biped"
analogy without settling the question) and `dep.pushkala-clause-scope` (`PD.06.Pushkala`'s own one
remaining blocker, registered by Milestone 40 and itself re-investigated by Milestone 41, which
found one same-chapter parallel construction insufficient to settle it either). Both entries have
now each been the sole, narrowly-scoped subject of their own dedicated re-investigation milestone
and both remain genuinely unresolved reading questions, not builds a future session should expect
to close by trying harder at the same search — see Decision 0d (this section) and Milestone 41's own
§C write-up for what has already been checked.
...
entirely (everything else left in the chapter needs a registered, unbuilt dependency — see the
header's own resume point). **Equally defensible: open a fresh chapter** — Ch.12 (Progeny), Ch.15
(Assessment of houses), Ch.16 (General house effects), Ch.27 (Sanyasa yogas), or the newly-unblocked
Ch.11 (Female Horoscopy)/Ch.18 (conjunction effects) — smaller, self-contained material with no
compounding per-verse judgement calls of chapter 7's own remaining kind. None of the three is
forced; a human should pick based on appetite: "finish a cheap reading call", "close out chapter 7's
own last easy verse", or "start clean".

**One further alternative, carried forward unchanged:**

1. **`concept:parallel-of-overloaded`** (Decision 0a) — an encoding pass over chapter 6's notes
